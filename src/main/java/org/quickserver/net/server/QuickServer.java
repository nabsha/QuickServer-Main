/*
 * This file is part of the QuickServer library Copyright (C) QuickServer.org
 * Use, modification, copying and distribution of this software is subject to
 * the terms and conditions of the GNU Lesser General Public License. You should
 * have received a copy of the GNU LGP License along with this library; if not,
 * you can download a copy from <http://www.quickserver.org/>. For questions,
 * suggestions, bug-reports, enhancement-requests etc. visit
...
/**
 * Main class of QuickServer library. This class is used to create multi client
 * servers quickly.
 * <p>
 * Ones a client is connected, it creates {@link ClientHandler} object, which is
 * run using any thread available from the pool of threads maintained by
 ...
 * &nbsp;Handles client interaction - writing data (Only used in non-blocking
 * mode).
 ...
 * The default DataMode for input is String so if not changes you will have to
 * set ClientCommandHandler.
 * </p>
 * <p>
 * Eg: <code><BLOCKQUOTE><pre>
package echoserver;
...
public class QuickServer implements Runnable, Service, Cloneable, Serializable {
  ...
  /**
   * Creates a new server without any configuration. Make sure you configure the
   * QuickServer, before calling startServer()
   * 
  ...
  /**
   * Creates a new server with the specified <code>commandHandler</code> has it
   * {@link ClientCommandHandler}.
   * 
   * @param commandHandler the fully qualified name of the desired class that
   ...
   * 
   * @param commandHandler fully qualified name of the class that implements
   *        {@link ClientCommandHandler}
  ...
  public QuickServer(ClientCommandHandler commandHandler, int port) {
    this.clientCommandHandler = commandHandler;
    setPort(port);
  }
   ...
   * 
   * @exception org.quickserver.net.AppException if Server already running or if
   *            it could not load the classes [ClientCommandHandler,
   *            ClientAuthenticationHandler, ClientData].
   * @see #startService
   */
  public void startServer() throws AppException {
    ...
    try {
      ...
      if (getSecure().isLoad() == true) loadSSLContext(); // v1.4.0
      loadBusinessLogic();
   ...
   * 
   * @exception org.quickserver.net.AppException if could not stop server
   * @since 1.1
   * @see #stopService
   */
   ...
   * 
   * @exception org.quickserver.net.AppException if could not stop server or if
   *            it could not start the server.
   * @since 1.2
   */
  ...
  /**
   * Sets the serverBanner for the QuickServer that will be displayed on the
   * standard output [console] when server starts. <br>
   * &nbsp;<br>
  ...
  /**
   * Sets the port for the QuickServer to listen on. If not set, it will run on
   * Port 9876
   * 
   * @param port to listen on.
   * @see #getPort
   ...
   * 
   * @param handler the fully qualified name of the class that implements
   *        {@link ClientCommandHandler}
  ...
  /**
   * Sets the ClientAuthenticationHandler class that handles the authentication
   * of a client.
   * 
   * @param authenticator the fully qualified name of the class that implements
   *        {@link ClientAuthenticationHandler}.
   ...
   * 
   * @param authenticator the fully qualified name of the class that implements
   *        {@link Authenticator} or {@link ClientAuthenticationHandler}.
  ...
  /**
   * Returns the Authenticator class that handles the authentication of a
   * client.
   * 
   ...
   * 
   * @param data the fully qualified name of the class that extends
   *        {@link ClientData}.
   * @see #getClientData
   */
   ...
   * 
   * @return the fully qualified name of the class that implements
   *         {@link ClientData}.
   * @see #setClientData
   */
  ...
  /**
   * Sets message to be displayed when maximum allowed login attempts has
   * reached. Default is : -ERR Max Auth Try Reached
   * 
  ...
  /**
   * Returns message to be displayed when maximum allowed login attempts has
   * reached.
   * 
   * @see #getMaxAuthTryMsg
   */
  ...
  public void run() {
    ...
    try {
      ...
      exceptionInRun = e;
      logger.log(Level.SEVERE, "{0} BindException for Port {1} @ {2} : {3}", new Object[] { getName(), getPort(),
  ...
  /**
   * Sets the message to be sent to any new client connected after maximum
   * client connection has reached. Default is :
   * <code>-ERR Server Busy. Max Connection Reached</code>
   * 
   * @since 1.1
  ...
  /**
   * Returns the message to be sent to any new client connected after maximum
   * client connection has reached.
   * 
   * @since 1.1
   ...
   *        QuickServer that will only accept connect requests to one of its
   *        addresses. If not set, it will default accepting connections on
   *        any/all local addresses.
   * @exception java.net.UnknownHostException if no IP address for the host
   *            could be found
   * @since 1.1
   * @see #getBindAddr
   */
  ...
  /**
   * Sets the store of objects to QuickServer, it is an array of objects that
   * main program or the class that created QuickServer passes to the
   * QuickServer.
   * 
  ...
  /**
   * Returns store of objects from QuickServer, if nothing was set will return
   * <code>null</code>.
   * 
   * @see #setStoreObjects
   * @since 1.1
  ...
  /**
   * Set the ClientAuthenticationHandler class of QSAdminServer that handles the
   * authentication of a client.
   * 
   * @since 1.2
   */
   ...
   *        handles the authentication of a client, if null uses
   *        {@link org.quickserver.net.qsadmin.Authenticator}.
  ...
  /**
   * Returns the application logger associated with QuickServer. If it was not
   * set will return QuickServer's own logger.
   * 
   * @since 1.2
   */
  ...
  /**
   * Sets the ClientObjectHandler class that interacts with client sockets to
   * handle java objects.
   * 
   * @param handler object the fully qualified name of the class that implements
   *        {@link ClientObjectHandler}
   ...
   * 
   * @param param of the xml configuration file.
   * @exception AppException if QuickServerConfig creation failed from the xml
   *            config file.
   * @since 1.4.7
   */
  ...
  /**
   * Returns the state of the process As any constant of {@link Service}
   * interface.
   * 
   * @since 1.2
   */
  ...
  /**
   * Sets the state of the process As any constant of {@link Service} interface.
   * 
   * @since 1.2
   */
  ...
  public void configQuickServer(QuickServerConfig config) throws Exception {
    ...
    else if (config.getClientAuthenticationHandler() != null) qs.setClientAuthenticationHandler(config.getClientAuthenticationHandler()); // v1.4.6
  ...
  /**
   * Configures QSAdminServer and QuickServer based on the internal
   ...
   * -load <xml_config_file> [options] Loads the server from xml file. where
   * options include: -fullXML2File <new_file_name>
   */
  public static void main(String args[]) {
    try {
      if (args.length >= 1) {
  ...
  /**
   * Returns ObjectPool of {@link org.quickserver.net.server.ClientData} class.
   * If ClientData was not poolable will return null.
   * 
   * @since 1.3
   */
   ...
   * 
   * @return DBPoolUtil object if object could be loaded, else will return
   *         <code>null</code>
   * @since 1.3
   */
   ...
   * <p>
   * Note: This command is an expensive so do use it limitedly and cache the
   * returned object. But before you start sending message to the cached object
   * do validate that ClientHandler with you is currently connected and is
   * pointing to the same clinet has it was before. This can be done as follows.
   * 
   * <pre>
   ...
   * {@link org.quickserver.net.server.ClientHandler} that are currently
   * handling clients. It is recommended not to change the collection under an
   * iterator. It is imperative that the user manually synchronize on the
   * returned collection when iterating over it: <code><pre>
   Eg:
   ...
   * returned object. But before you start sending message to the cached object
   * do validate that ClientHandler with you is currently connected and is
   * pointing to the same client has it was before. This can be done as follows.
   * 
   * <pre>
	foundClientHandler.isConnected(); //this method will through SocketException if not connected
	Date newTime = foundClientHandler.getClientConnectedTime();
   ...
   * <p>
   * This will skip any authentication and will not set any timeout.
   * </p>
   * 
   * @since 1.3.2
   */
   ...
   * 
   * @param securityManagerClass the fully qualified name of the class that
   *        extends {@link java.lang.SecurityManager}.
  ...
  /**
   * Gets the classloader used to load the dynamically resolved classes.
   * 
   * @since 1.4.6
   */
      ...
      try {
        ...
        if (getBlockingMode() == false) {
          ...
          SSLServerSocket serversocket =
              (SSLServerSocket) ssf.createServerSocket(getPort(), getBasicConfig().getAdvancedSettings().getBacklog(), getBindAddr());
  ...
  /**
   * Sets the ClientBinaryHandler class that interacts with client sockets to
   * handle binary data.
   * 
   * @param handler object the fully qualified name of the class that implements
   *        {@link ClientBinaryHandler}
   ...
   * 
   * @return boolean flag to indicate if new entry was added to the list to
   *         register.
   * @since 1.4.5
   */
  public boolean registerChannel(SocketChannel channel, int ops, Object att) throws IOException, ClosedChannelException {
    ...
    /*
     * logger.warning("Before register..."); channel.register(getSelector(),
     * ops, att); logger.warning("Before wakeup and after register...");
     * getSelector().wakeup(); logger.warning("After wakeup...");
     */
  }
  ...
  /**
   * Sets the ClientWriteHandler class that interacts with client sockets to
   * handle data write (only used in non-blocking mode).
   * 
   * @param handler object the fully qualified name of the class that implements
   *        {@link ClientWriteHandler}
  ...
  /**
   * Sets the debug flag to ByteBufferOutputStream and ByteBufferInputStream
   * class that are used in non-blcking mode
   * 
   * @since 1.4.5
   */
  ...
  /**
   * Sets the ClientExtendedEventHandler class that gets notified of extended
   * client events.
   * 
   * @param handler the fully qualified name of the class that implements
   *        {@link ClientExtendedEventHandler}
  ...
  /**
   * Returns the ClientExtendedEventHandler class that gets notified of extended
   * client events.
   * 
